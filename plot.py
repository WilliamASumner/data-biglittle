...
def main():
    ...
    # Layout for the data:
    # websiteData[coreConfig][govConfig][siteName][loadTimeType] -> npArray of values
    bySite = dict(zip(sites,[dict() for site in sites]))
    byGov= dict(zip(govConfigs,[bySite.deepcopy() for config in govConfigs]))
    websiteData = dict(zip(coreConfigs,
        [byGov.deepcopy() for config in coreConfigs]))
